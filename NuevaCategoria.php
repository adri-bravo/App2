...
if (isset($email, $password)) {
    ...
    if (login($email, $password) == true) {
		...
		$errores = 0;
		foreach ($obj['Guardar'] as $k=>$Categoria)
		{	
		
			 $sql = "INSERT INTO Categorias(Id_Usuario, Categoria) VALUES (".$GL_id_usuario.",'" 
			 .$Categoria{'Categoria'}."')";
			 if (mysql_query( $sql,$link)==true)
			 {
				if(mysql_affected_rows()==-1){
					$errores = $errores +1;
					$lista_errores[] = array('Error' =>"Categorias:".$Categoria{'Categoria'});
					}
			 }else{$errores = $errores +1;$lista_errores[] = array('Error' =>"Categorias:".$Categoria{'Categoria'});}
		}
		if($errores == 0){
			$resultado = "OK";
			mysql_query("COMMIT");
